...
function initEcho() {
    ...
    const host    = pusherConfig.host || '';
    const port    = Number(pusherConfig.port || 443);
    const scheme  = pusherConfig.scheme || 'https';
    ...
    // Reverb speaks the Pusher protocol but is hosted on our own domain.
    // Supplying these values at runtime keeps the same frontend build valid
    // when the production hostname changes.
    if (host) {
