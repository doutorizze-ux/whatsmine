    ...
    private function pusherPublicConfig(): array
    {
        try {
            ...
            $key = $usesReverb
                ...
                : (SystemSetting::get('pusher_app_key') ?: env('PUSHER_APP_KEY', ''));
            $cluster = $usesReverb
                ? 'mt1'
                : (SystemSetting::get('pusher_app_cluster') ?: env('PUSHER_APP_CLUSTER', 'mt1'));
            ...
            return [
                ...
                'cluster' => $cluster,
                'enabled' => $enabled,
                'host' => $usesReverb ? env('REVERB_HOST', '') : env('PUSHER_HOST', ''),
                'port' => (int) ($usesReverb ? env('REVERB_PORT', 443) : env('PUSHER_PORT', 443)),
                'scheme' => $usesReverb ? env('REVERB_SCHEME', 'https') : env('PUSHER_SCHEME', 'https'),
            ];
        } catch (\Throwable) {
